...
$OutputJar = Join-Path $BuildDir "BonfireItemDamaged-1.1.1-fix1.jar"
...
$ItemsAdderJar = (Get-ChildItem -Path $PluginsDir -Filter "*ItemsAdder*.jar" -File -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1).FullName
...
$TargetPluginJar = (Get-ChildItem -Path $PluginsDir -File -ErrorAction SilentlyContinue | Where-Object { $_.Name -like "*BonfireItemDamaged*.jar" } | Sort-Object LastWriteTime -Descending | Select-Object -First 1).FullName
...
if ([string]::IsNullOrWhiteSpace($ItemsAdderJar) -or !(Test-Path $ItemsAdderJar)) {
    throw "Missing ItemsAdder jar: $ItemsAdderJar"
}
...
if ([string]::IsNullOrWhiteSpace($TargetPluginJar) -or !(Test-Path $TargetPluginJar)) {
    throw "Missing target BonfireItemDamaged plugin jar in: $PluginsDir"
}
...
$Classpath = @($ApiJar, $ItemsAdderJar, $NbtApiJar) + $KyoriJars -join ";"
